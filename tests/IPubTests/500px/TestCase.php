<?php
/**
 * Test: IPub\FiveHundredPixel\ConfigurationTest
 * @testCase
 *
...
class TestCase extends Tester\TestCase
{
	...
	protected function buildClient($query = [])
	{
		...
		$this->config = new IPub\FiveHundredPixel\Configuration('123', 'abc');
		...
		$this->session = new IPub\FiveHundredPixel\SessionStorage($session, $this->config);

		$this->httpClient = new ApiClientMock();

		$consumer = new OAuth\Consumer('123', 'abc');

		return new IPub\FiveHundredPixel\Client($consumer, $this->httpClient, $this->config, $this->session, $httpRequest);
	}
}
